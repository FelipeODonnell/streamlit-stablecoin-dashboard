...
from utils.security import InvalidURLError, RateLimitExceededError, ValidationError
    ...
    while attempt < retries:
        try:
            ...
            if attempt < retries:
                ...
                handle_api_error(
                    e, 
                    f"Failed to parse JSON from {url} after {retries} attempts", 
                    logging.ERROR
                )
                return None
...
def get_stablecoin_metadata(
    ...
    try:
        ...
        handle_api_error(
            e, 
            "Error processing stablecoin metadata", 
            logging.ERROR,
            show_traceback=True
        )
        return pd.DataFrame()
