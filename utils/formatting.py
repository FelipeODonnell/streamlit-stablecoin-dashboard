...
def categorize_stablecoin_by_strategy(desc):
    ...
    if (
        "delta neutral" in desc
        or "funding" in desc
        or "cash & carry" in desc
        or "delta neutra" in desc
        or "perps funding" in desc
    ):
        return "Delta Neutral/Arb"
    if "restaking" in desc:
        return "Restaking"
    if (
        "native protocol yield" in desc
        or "protocol revenue" in desc
        or "interest paid by" in desc
        or "safety module" in desc
        or "native dex" in desc
        or "sky savings rate" in desc
    ):
